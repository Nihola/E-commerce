...
const SumDelivery = ({ handleSubmit, sendMessage }) => {
  ...
  return (
    <section className='w-[400px] bg-gray-100 border border-gray-400 py-4 px-6 rounded-lg'>
      <div className='flex items-center justify-center gap-4 py-1 text-center mx-auto'>
        <div onClick={() => setIsOn(!isOn)} className={`w-14 h-8 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${isOn ? 'bg-green-500' : ''}`}>
      ...
      <div className='py-4 space-y-2'>
        <span className='flex justify-between gap-4 py-0.5 border-b border-b-gray-300'>
          <p className='text-xl text-gray-800'>3 ta mahsulot</p>
      ...
      <button type='submit' onClick={handleSubmit(sendMessage)} className='w-full border p-3 bg-green-500 text-white rounded-lg font-semibold cursor-pointer hover:bg-green-400 active:scale-96 duration-250'>Yetkazib bergandan keyin to'lov</button>
    </section>
  )
}
